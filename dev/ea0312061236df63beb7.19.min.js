...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./drapCanvas.less", function() {
			var newContent = require("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./drapCanvas.less");
  ...
  _createClass(CanvasBase, [{
    ...
    value: function series_get() {
      ...
      this.api.series_get({
        ...
        for (var i = 0; i < serLength; i++) {
          seriesInstanceUids.push({ seriesAnnotationResultId: value.data.sarIdList[i], seriesInstanceUid: value.data.seriesList[i] });
        }
        var endInfo = { studyInstanceUid: value.data.studyInstanceUid, seriesInstanceUids: seriesInstanceUids };
        ...
        _this3.model.setData('series_result_id', endInfo);
      });
    }
  }, {
    key: 'start_series',
    ...
    value: function cornerstoneContorlOther() {
      ...
      this.cornerstoneContorl.event._addEvent('ctcornerstone.Allthing', function (value) {
        _this4.app.loading.show();
        var ids = _this4.getSarIds();
        var data = _this4.model.changeItemDataToBackend(_this4.model.taskInfo.id, value);
        //data.taskId = that.model.taskInfo.id
        //data.studyInstanceUid = that.model.series_result_id.studyInstanceUid
        data.sarIdList = ids;
        _this4.api.series_result_item_edit(data).done(function () {
          _this4.app.loading.hide();
        });
      });
      //获取某一个病症的征象信息
      this.cornerstoneContorl.event._addEvent('ctcornerstone.editNodeItem', function (value) {
        _this4.app.loading.show();
        _this4.api.image_result_item_edit(_this4.model.changeItemDataToBackend(value.backId, value.chooseData)).done(function () {
          _this4.app.loading.hide();
        });
      });
    }
    ...
    value: function submitSeries(id, pading) {
      ...
      this.api.series_result_read({
        ...
        console.log(value, 'valuevaluevaluevalue', value.data.yayAttributes, _this7.model.projectInfo);
        if (_this7.model.projectInfo.yayAttributes == value.data.yayAttributes) {
          _this7.startSubmit();
          return false;
        }
        _this7.app.loading.hide();
        var data = value.data;
        var conmponentInfo = _this7.cornerstoneContorl.model.nidusComponentInfo;
        //判断大征象是否完成了
        var allnid = _this7.allnidussubmit(data);
        if (!allnid) {
          return true;
        }
          ...
          if (item.annotationItemResultList == null && optionLength != 0) {
            for (var w in conmponentInfo[item.imageAnnotationType]) {
              if (conmponentInfo[item.imageAnnotationType][w].optional) {
                _this7.errorShow(item, conmponentInfo[item.imageAnnotationType][w]);
                pading = true;
                return true;
              }
            }
          ...
          if (item.annotationItemResultList) {
            ...
            if (optionLength) {
              ...
              for (var _w in conmponentInfo[item.imageAnnotationType]) {
                if (idsAll.lastIndexOf(',' + _w + ',') == -1 && conmponentInfo[item.imageAnnotationType][_w].optional) {
                  _this7.errorShow(item, conmponentInfo[item.imageAnnotationType][_w]);
                  pading = true;
                  return true;
                }
              }
            ...
            for (var _j = 0; _j < item.annotationItemResultList.length; _j++) {
              ...
              if (!_res.result && conmponentInfo[item.imageAnnotationType][_res.annotationItemId].optional) {
                pading = true;
                _this7.errorShow(item, conmponentInfo[item.imageAnnotationType][_res.annotationItemId]);
                return true;
              }
            }
          }
      ...
      var _loop2 = function _loop2(i) {
        if (i != 'annotationItemResultList') {
          ...
          if (allitem.optional) {
            ...
            if (!nowS) {
              _this8.alerError('“序列整体标注信息”的“' + allitem.componentName + '”为必填项，请填写完整。');
              return {
                v: false
              };
            }
    ...
    value: function series_submit() {
      ...
      this.api.series_result_submit({
        ...
        console.log(_this9.model.doneNum);
        //return
        if (_this9.model.doneNum == -1) {
          window.location.href = 'http://' + window.location.host + '/#!/personalaccount';
          window.location.reload();
...
//# sourceMappingURL=ea0312061236df63beb7.19.min.js.map