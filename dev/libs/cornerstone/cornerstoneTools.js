...
function pointNearEllipse(element, data, coords, distance) {
	var cornerstone = _externalModules2.default.cornerstone;
	  ...
	  var  minorEllipseCanvWidth = Math.abs(startCanvas.x - endCanvas.x) - distance;
	  var  minorEllipseCanvHeight = Math.abs(startCanvas.y - endCanvas.y) - distance;
	  if(minorEllipseCanvWidth  >= minorEllipseCanvHeight ) {
		    minorEllipseCanvHeight = minorEllipseCanvWidth; 
		} else {
			minorEllipseCanvWidth = minorEllipseCanvHeight;
		}
	  var minorEllipse = {
	    left: Math.min(startCanvas.x, endCanvas.x) + distance / 2,
	    top: Math.min(startCanvas.y, endCanvas.y) + distance / 2,
	    width: minorEllipseCanvWidth,
	    height: minorEllipseCanvHeight
	  };
	  
	  
	  var  majorEllipseCanvWidth =  Math.abs(startCanvas.x - endCanvas.x) + distance;
	  var  majorEllipseCanvHeight = Math.abs(startCanvas.y - endCanvas.y) + distance;

	  if(majorEllipseCanvWidth  >= majorEllipseCanvHeight ) {
		  majorEllipseCanvHeight = majorEllipseCanvWidth; 
		} else {
			majorEllipseCanvWidth = majorEllipseCanvHeight;
		}
	  ...
	  var majorEllipse = {
	    left: Math.min(startCanvas.x, endCanvas.x) - distance / 2,
	    top: Math.min(startCanvas.y, endCanvas.y) - distance / 2,
	    width: majorEllipseCanvWidth,
	    height: majorEllipseCanvHeight
	  };
...
function onImageRendered(e, eventData) {
  ...
  for (var i = 0; i < toolData.data.length; i++) {
    ...
    data.handles.start.widthCanvas = widthCanvas.toFixed(2);
    ...
    if (data.invalidated === false) {
      ...
      data.handles.start.areamm = area.toFixed(2);
