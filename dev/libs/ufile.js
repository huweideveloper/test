...
UCloudUFile.prototype.getContentMd5 = function(file, success) {
  ...
  function loadNext() {
    ...
    var end =
      start + that.sliceSize >= file.size ? file.size : start + that.sliceSize
...
// 批量上传
UCloudUFile.prototype.batchUpload = function(
  fileList,
  success,
  error,
  progress
...
// 分片上传（外部调用）
UCloudUFile.prototype.sliceUpload = function(
  options,
  success,
  error,
  progress
...
// 初始化上传（内部调用）
UCloudUFile.prototype.initMultipartUpload = function(
  success,
  error,
  progress,
  file,
...
// 上传分片（内部调用）
UCloudUFile.prototype.multipartUploading = function(
  success,
  error,
  keyName,
  uploadId,
...
// 完成分片（内部调用）
UCloudUFile.prototype.multipartUploaded = function(
  success,
  error,
  progress,
  keyName,
...
// 下载文件
UCloudUFile.prototype.downloadFile = function(
  fileName,
  success,
  error,
  progress
