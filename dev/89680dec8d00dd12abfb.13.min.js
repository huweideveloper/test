...
module.exports = "<div class=mark> <ul class=mark-header> <li class=\"back-icon fl\"> <span><span class=lefta><i class=\"iconfont icon-icon-test3\" style=font-size:12px></i></span><span class=infojian>医生主页</span></span> </li> <li class=\"task-name fl\"> <span>|</span> <span>Cassiel0705标注4</span> </li> <li class=\"cotrol-btn fr\"> <div class=\"btn btn-pause\">暂停</div> <div class=\"btn btn-submit\">提交</div> </li> <li class=\"discard_info fr\" style=margin-right:10px></li> </ul> <div class=mark-menu></div> <div class=mark-content> </div> </div>";
...
var markview = function (_MarkBase) {
    ...
    function markview(app, api, dom, model) {
        ...
        return _possibleConstructorReturn(this, (markview.__proto__ || Object.getPrototypeOf(markview)).call(this, app, api, dom, model));
    }
    //接口调用，获取信息
    ...
    _createClass(markview, [{
        ...
        value: function editTask() {
            ...
            this.api.series_result_search({
                ...
                if (value.data.list.length != 0) {
                    ...
                    for (var _i = 0; _i < value.data.list.length; _i++) {
                        var data = value.data.list[_i];
                        temp.studyInstanceUid = data.studyInstanceUid;
                        seriesInstanceUids.push({
                            seriesAnnotationResultId: data.id,
                            seriesInstanceUid: data.seriesInstanceUid,
                            needAnno: value.data.seriesList[_i].needAnno
                        });
                    }
        ...
        value: function startTask() {
            ...
            this.api.series_get({
                ...
                var seriesInstanceUids = [];
                for (var _i2 = 0; _i2 < serLength; _i2++) {
                    seriesInstanceUids.push({
                        seriesAnnotationResultId: value.data.sarIdList[_i2],
                        seriesInstanceUid: value.data.seriesList[_i2].series,
                        needAnno: value.data.seriesList[_i2].needAnno,
                        imageList: value.data.seriesList[_i2].imageIdList
                    });
                }
                var endInfo = {
        ...
        value: function result_get_fromback() {
            ...
            var _loop = function _loop(_i3) {
                _this5.api.series_result_read({
                    id: lists[_i3].seriesAnnotationResultId
                }).done(function (res) {
                    console.log(res, '我是是是是是');
                    if (res.data.discardCode) {
                        var str = '';
                        str += res.data.discardReason ? res.data.discardReason : "";
                        _this5.dom.find('.discard_info').html('<span>当前序列被标为废片，原因：' + str + '</span>');
                    }
                    _this5.model.series_result_id.remark = 0;
                    _this5.model.seriesResult[lists[_i3].seriesAnnotationResultId] = res;
                    _this5.model.setData('seriesResult', _this5.model.seriesResult);
                });
            };
        ...
        value: function cornerstoneContorlOther() {
                ...
                that.app.alert.show({
                    ...
                    sure: function () {
                        var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
                            var ctVal, points, backData, magicloadingClass, j, resCreate, resRead;
                                ...
                                while (1) {
                                    switch (_context.prev = _context.next) {
                                        case 0:
                                            ...
                                            _context.t0 = regeneratorRuntime.keys(that.model.seriesInfo);
            ...
            this.cornerstoneContorl.event._addEvent('ctcornerstone.editBrush', function (value) {
                ...
                if (value.toolType.type == 'POLYGON' || value.toolType.type == 'QSELECT' || value.toolType.type == 'REGION_PAINT' || value.toolType.type == 'FREEHAND') {
                    ...
                    that.app.loading.show();
                    var _base = value.image.src.replace('data:image/png;base64,', '');
                    ...
                    var layerNumber = value.layerNumber;
                ...
                var base64 = Tool.changeImageToBase64(value.image);
            ...
            this.cornerstoneContorl.event._addEvent('ctcornerstone.addImageResult', function (value) {
                ...
                var rid = void 0;
                for (var _i4 in that.model.seriesResult) {
                    if (that.model.seriesResult[_i4].data.seriesInstanceUid == value.sId) {
                        rid = _i4;
                    }
                }
                setTimeout(function () {
        ...
        key: 'needApiAll',
        value: function needApiAll(data, apineed, newdata) {
        ...
        value: function btnEvent() {
            ...
            this.dom.find('.cotrol-btn .btn-pause').on('click', function () {
                _this7.api.series_result_act_record({
                    userId: _this7.useinfo.userId,
                    taskId: _this7.app.parpam['taskId'],
                    sarIdList: _this7.getSarIds(),
                    userRecord: 2
                });
        ...
        value: function submitSeries(id, pading) {
            ...
            this.api.series_result_read({
                ...
                _this8.app.loading.hide();

                _this8.needSub[vid] = false;
                if (value.data.yayAttributes) {
                    if (_this8.model.projectInfo.yayAttributes * 1 == 4) {
                        _this8.startSubmit();
                        return false;
                    }
                    if (_this8.model.projectInfo.yayAttributes * 1 == 3 && value.data.yayAttributes * 1 == 1) {
                        _this8.startSubmit();
                        return false;
                    }
                    if (_this8.model.projectInfo.yayAttributes * 1 == 1 && value.data.yayAttributes * 1 == 2) {
                        _this8.startSubmit();
                        return false;
                    }
                }
                var data = value.data;
                var conmponentInfo = _this8.cornerstoneContorl.model.nidusComponentInfo;
                //判断大征象是否完成了
                var allnid = _this8.allnidussubmit(data);
                if (!allnid) {
                    return true;
                }
                console.log(conmponentInfo, 'conmponentInfoconmponentInfoconmponentInfo');
                console.log(_this8.needSub, vid);
                _this8.needSub[vid] = false;
                for (var _i5 = 0; _i5 < data.imageAnnotationResultList.length; _i5++) {
                    var item = data.imageAnnotationResultList[_i5];
                    var optionLength = 0;
                    ...
                    if (item.annotationItemResultList == null && optionLength != 0) {
                        for (var w in conmponentInfo[item.imageAnnotationType]) {
                            if (conmponentInfo[item.imageAnnotationType][w].optional) {
                                _this8.errorShow(item, conmponentInfo[item.imageAnnotationType][w]);
                                _this8.needSub[vid] = true;
                                return true;
                            }
                        }
                    }
                    ...
                    if (item.annotationItemResultList) {
                        ...
                        if (optionLength) {
                            ...
                            for (var _w in conmponentInfo[item.imageAnnotationType]) {
                                if (idsAll.lastIndexOf(',' + _w + ',') == -1 && conmponentInfo[item.imageAnnotationType][_w].optional) {
                                    _this8.errorShow(item, conmponentInfo[item.imageAnnotationType][_w]);
                                    _this8.needSub[vid] = true;
                                    return true;
                                }
                            }
                        ...
                        for (var _j = 0; _j < item.annotationItemResultList.length; _j++) {
                            ...
                            if (!_res.result && conmponentInfo[item.imageAnnotationType][_res.annotationItemId].optional) {
                                _this8.needSub[vid] = true;
                                _this8.errorShow(item, conmponentInfo[item.imageAnnotationType][_res.annotationItemId]);
                                return true;
                            }
                        }
                    }
        ...
        value: function allnidussubmit(data) {
            ...
            var _loop2 = function _loop2(_i6) {
                if (_i6 != 'annotationItemResultList') {
                    var allitem = conmponentInfo.all[_i6];
                    if (allitem.optional) {
                        var nowS = false;
                        data.annotationItemResultList.map(function (item) {
                            if (item.annotationItemId == _i6 && item.result) {
                                nowS = true;
                            }
                        });
                        if (!nowS) {
                            _this9.alerError('“序列整体标注信息”的“' + allitem.componentName + '”为必填项，请填写完整。');
                            return {
                                v: false
                            };
                        }
            ...
            for (var _i6 in conmponentInfo.all) {
                var _ret2 = _loop2(_i6);
        ...
        value: function series_submit() {
            ...
            this.api.series_result_submit({
                ...
                _this10.app.loading.hide();
                if (_this10.model.doneNum == -1) {
                    window.location.href = 'http://' + window.location.host + '/#!/personalaccount';
                    window.location.reload();
...
var api = {
  ...
  series_result_act_record: function series_result_act_record(value) {
  ...
  //项目基础信息
  alg_sar_reset_mask_image: function alg_sar_reset_mask_image(value) {
    var st = api.app.tokentime();
    if (!st) {
      delete value.token;
    }
  ...
  // },
  // 魔法棒创建
  magicToolCerate: function magicToolCerate(data) {
    return this.HttpRequest.POST('/mask/magic_stick/create', data);
  },

  // 魔法棒查看一个
  magicToolRead: function magicToolRead(data) {
  ...
  // },
  //魔法棒删除
  image_process_delete: function image_process_delete(value) {
    var deferred = ES.Deferred();
    ES.ajax({
...
//# sourceMappingURL=89680dec8d00dd12abfb.13.min.js.map