...
var markseriesview = function (_MarkBase) {
    _inherits(markseriesview, _MarkBase);

    function markseriesview(app, api, dom, model) {
        _classCallCheck(this, markseriesview);

        var _this = _possibleConstructorReturn(this, (markseriesview.__proto__ || Object.getPrototypeOf(markseriesview)).call(this, app, api, dom, model));
    ...
    _createClass(markseriesview, [{
        ...
        value: function apiload() {
            ...
            this.api.series_review_task_read({ id: this.app.parpam['taskId'] }).done(function (value) {
                _this2.app.loading.hide();
                _this2.model.setData('projectInfo', value.data);
                _this2.conclusion = value.data.conclusion;
                _this2.suitable = '' + value.data.suitable;
                _this2.dom.find('.task-name span').eq(1).html('适用项目：' + value.data.title);
                _this2.dom.find('.dicommenu .btn').addClass('hide');
            });
        }
    }, {
        key: 'seriesInfo',
        value: function seriesInfo() {
            var sid = this.model.series_result_id.studyInstanceUid;
        ...
        value: function annotationIteminit() {
            ...
            this.api.series_review_task_series_search(this.model.frompage).done(function (value) {
                _this3.app.loading.hide();
                if (value.data.list.length != 0) {
                    _this3.series_get(value.data.list[0]);
                    _this3.totalseries = value.data.total;
                    _this3.dom.find('.shownum .total').html(_this3.totalseries);
                    _this3.dom.find('.shownum .now').html(_this3.app.parpam['sid']);
                } else {
                    //当拿不到序列数据的时候自动帮忙跳回去原来的页面，下面的链接需要改
                    window.location.href = 'http://' + window.location.host + '/#!/cleardata/' + _this3.app.parpam['taskId'];
        ...
        value: function series_get(sid) {
            ...
            console.log(sid);
            var temp = { studyInstanceUid: sid, seriesInstanceUids: [{ seriesAnnotationResultId: 30, seriesInstanceUid: sid }] };
        ...
        value: function btnEvent() {
            ...
            this.dom.find('.back-icon').on('click', function () {
                if (_this5.app.parpam['type'] != "viewer") {
                    window.location.href = 'http://' + window.location.host + '/#!/cleardata/' + _this5.app.parpam['taskId'];
                    window.location.reload();
                    // this.app.changePage('ytjtaskdetail', { taskid: this.app.parpam['taskId'] })
                    return;
                } else {
                    window.location.href = 'http://' + window.location.host + '/#!/cleardata/' + _this5.app.parpam['taskId'] + '/' + _this5.app.parpam['rid'];
                    window.location.reload();
                    //this.app.changePage('taskdetail', { taskId: this.app.parpam['taskId'], type: this.app.parpam['rid'] })
                    return;
                }
            });
            this.dom.find('.cotrol-btn .btn-up').on('click', function () {
                if (_this5.app.parpam['sid'] * 1 == 1) {
                    _this5.alerError('对不起前面没有序列了！');
                    return;
                }
                window.location.href = 'http://' + window.location.host + '/#!/markseriesview/' + _this5.app.parpam['taskId'] + '/view' + '/' + (_this5.app.parpam['sid'] * 1 - 1);
                window.location.reload();
            });
            this.dom.find('.jumpto').on('keydown', function () {
                if (event.keyCode === 13) {
                    var temp = parseInt(ES.selctorDoc(this).val());
                    if (temp < 1 || temp > that.totalseries || isNaN(temp)) {
                        that.alerError('没有该序列');
                        return;
                    }
                    window.location.href = 'http://' + window.location.host + '/#!/markseriesview/' + that.app.parpam['taskId'] + '/view' + '/' + temp;
                    window.location.reload();
                }
            });
            this.dom.find('.cotrol-btn .btn-next').on('click', function () {
                console.log(that.totalseries, _this5.app.parpam['sid'] * 1);
                if (_this5.app.parpam['sid'] * 1 == _this5.totalseries) {
                    _this5.alerError('对不起后面没有序列了！');
                    return;
                }
                window.location.href = 'http://' + window.location.host + '/#!/markseriesview/' + _this5.app.parpam['taskId'] + '/view' + '/' + (_this5.app.parpam['sid'] * 1 + 1);
                window.location.reload();
            });
            this.dom.find('.cotrol-btn .btn-allse').on('click', function () {
                var formsubmit = __webpack_require__(783);
                var config = [{ name: 'conclusion', type: 'input', title: '提交批次结论', value: that.conclusion, check: 'empty', remark: '本批次是否适合该项目可以在此填写' }, { name: 'suitable', type: 'dropdown', title: '是否适合项目', value: that.suitable == true ? '1' : '0', 'check': 'empty', remark: '', data: [{ val: '是', idx: '1' }, { val: '否', idx: '0' }] }];
                var editconclusion = _this5.app.loadModal(formsubmit, { adv: true }, { title: '批次结论', config: config });
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./markseriesview.less", function() {
			var newContent = require("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./markseriesview.less");
...
var api = (_api = {
    ...
    //查询copd的值
    queryCopdVal: function queryCopdVal(iarId) {
    ...
    //任务读取
    series_review_task_read: function series_review_task_read(value) {
        var st = api.app.tokentime();
        if (!st) {
            delete value.token;
        }
    ...
    series_review_task_series_search: function series_review_task_series_search(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    return deferred;
}), _defineProperty(_api, 'series_review_task_update', function series_review_task_update(value) {
    var deferred = ES.Deferred();
    ES.ajax({
...
module.exports = "<div class=markseriesview> <ul class=mark-header> <li class=\"back-icon fl\"> <span><span class=lefta><i class=\"iconfont icon-icon-test3\" style=font-size:12px></i></span>返回</span> </li> <li class=\"task-name fl\"> <span>|</span> <span></span> </li> <li class=\"discard_info fl\" style=margin-left:10px></li> <li class=\"cotrol-btn fr\"> <div class=\"btn btn-up\">上一个序列</div> <div class=\"btn btn-next\">下一个序列</div> <div class=\"btn btn-allse\">提交此次结论</div> </li> <li class=\"shownum fr\"> <input class=jumpto type=text title=回车跳转至指定序列 /> <label class=now></label> <label>/</label> <label class=total></label> </li> </ul> <div class=mark-menu></div> <div class=mark-content> </div> </div>";
...
var markseriesview = function (_MarkModel) {
    _inherits(markseriesview, _MarkModel);

    function markseriesview(app) {
        _classCallCheck(this, markseriesview);

        return _possibleConstructorReturn(this, (markseriesview.__proto__ || Object.getPrototypeOf(markseriesview)).call(this, app));
    }
...
//# sourceMappingURL=4f4d2337772400783a95.7.min.js.map