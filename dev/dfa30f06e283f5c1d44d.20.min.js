...
var markmodel = function (_MarkModel) {
    _inherits(markmodel, _MarkModel);

    function markmodel(app) {
        _classCallCheck(this, markmodel);

        return _possibleConstructorReturn(this, (markmodel.__proto__ || Object.getPrototypeOf(markmodel)).call(this, app));
    }
    ...
    _createClass(markmodel, [{
        ...
        value: function makeJsonToBackend(value, handles, needbian) {
            var result = void 0;
            console.log(this.seriesInfo[value.sId].info.data);
            switch (value.type) {
                case "ELLIPSE":
                    var sp = this.seriesInfo[value.sId].info.data.pixelSpacing ? this.seriesInfo[value.sId].info.data.pixelSpacing[0] : 1;
                    result = {
                    ...
                    break;
                case "RECTANGLE":
                    var minx = Math.min(handles.start.x, handles.end.x);
                    var maxx = Math.max(handles.start.x, handles.end.x);
                    var miny = Math.min(handles.start.y, handles.end.y);
                    var maxy = Math.max(handles.start.y, handles.end.y);
                    var startAll = JSON.parse(JSON.stringify(handles.start));
                    startAll.x = minx;
                    startAll.y = miny;
                    if (needbian) {
                    ...
                    var endAll = JSON.parse(JSON.stringify(handles.end));
                    endAll.x = maxx;
                    endAll.y = maxy;
                    if (needbian) {
                        endAll.x = endAll.x * 2;
                    ...
                    break;
                case "ANGLE":
                    var startAll1 = JSON.parse(JSON.stringify(handles.start));
                    if (needbian) {
                        startAll1.x = startAll1.x * 2;
                        startAll1.y = startAll1.y * 2;
                    }
                    var middleAll1 = JSON.parse(JSON.stringify(handles.middle));
                    if (needbian) {
                        middleAll1.x = middleAll1.x * 2;
                        middleAll1.y = middleAll1.y * 2;
                    }
                    var endAll1 = JSON.parse(JSON.stringify(handles.end));
                    if (needbian) {
                ...
                case "COBB":
                    ...
                    console.log(value);
                    var nowH = JSON.parse(JSON.stringify(handles));
                    nowH.map(function (item) {
                        if (needbian) {
                    ...
                    break;
                case "ALIGNMENT":
                    var startAll12 = JSON.parse(JSON.stringify(handles.start));
                    if (needbian) {
                        startAll12.x = startAll12.x * 2;
                        startAll12.y = startAll12.y * 2;
                    }
                    var middleAll12 = JSON.parse(JSON.stringify(handles.middle));
                    if (needbian) {
                        middleAll12.x = middleAll12.x * 2;
                        middleAll12.y = middleAll12.y * 2;
                    }
                    var middleAll22 = JSON.parse(JSON.stringify(handles.middle1));
                    if (needbian) {
                        middleAll22.x = middleAll22.x * 2;
                        middleAll22.y = middleAll22.y * 2;
                    }
                    var endAll12 = JSON.parse(JSON.stringify(handles.end));
                    if (needbian) {
        ...
        key: 'getpoints',
        value: function getpoints(value, type) {
            var bord = null;
            switch (type) {
                case "ANNO4":
                    bord = 130;
            ...
            for (var i = 0; i < value.points.length; i++) {
                var ct = (this.seriesInfo[value.sId].info.wwc.hight * 1 - this.seriesInfo[value.sId].info.wwc.low * 1) * value.points[i].ct / 255 + this.seriesInfo[value.sId].info.wwc.low * 1;
                console.log(ct);
                if (bord) {
                    if (ct > bord) {
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./doctorAudEdit.less", function() {
			var newContent = require("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./doctorAudEdit.less");
...
var api = {
    ...
    //项目序列列表
    project_series_search: function project_series_search(value) {
        var st = api.app.tokentime();
        if (!st) {
            delete value.token;
        }
    ...
    //序列原始tag
    series_origin_detail: function series_origin_detail(value) {
        var st = api.app.tokentime();
        if (!st) {
            delete value.token;
        }
    ...
    //缓存序列
    series_cache: function series_cache(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    image_result_update: function image_result_update(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    image_result_delete: function image_result_delete(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    //****************单个标注结果*********************
    //组件结果上传
    image_result_item_edit: function image_result_item_edit(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    //废片结果
    series_discard: function series_discard(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    //魔法棒接口
    image_process_create: function image_process_create(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    //魔法棒批量查看
    image_process_batch_read: function image_process_batch_read(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    //魔法棒接口
    image_process_item_edit: function image_process_item_edit(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    anno_iar_create: function anno_iar_create(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    anno_iar_update: function anno_iar_update(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    anno_iar_delete: function anno_iar_delete(value) {
        var deferred = ES.Deferred();
        ES.ajax({
...
var markview = function (_MarkBase) {
    _inherits(markview, _MarkBase);

    function markview(app, api, dom, model) {
        _classCallCheck(this, markview);

        var _this = _possibleConstructorReturn(this, (markview.__proto__ || Object.getPrototypeOf(markview)).call(this, app, api, dom, model));

        _this.app.parpam['sid'] = Base64.decode(_this.app.parpam['sid']);
        return _this;
    }
    //接口调用，获取信息
    ...
    _createClass(markview, [{
        ...
        value: function apiload() {
            ...
            this.api.task_read({
                ...
                _this2.model.taskInfo.studyAnno = value.data.studyAnno;
            });
            // console.log(this.dom.find("#header")，)
            // return
        ...
        value: function series_get(sid) {
            ...
            this.api.series_result_search({
                ...
                if (value.data.list.length != 0) {
                    ...
                    if (!_this3.model.taskInfo.studyAnno) {
                    ...
                    _this3.model.setData('series_result_id', temp);
                } else {
                    _this3.dom.find('.back-icon').click();
                }
            });
        }
        // 编辑
        ...
        value: function editTask() {
            ...
            this.api.series_result_search({
                ...
                console.log(value, '我是编辑人物过来的东西');
                _this4.app.loading.hide();
                if (value.data.list[0].discardCode) {
                    var str = '';
                    str += value.data.list[0].discardReason ? value.data.list[0].discardReason : "";
                    _this4.dom.find('.discard_info').html('<span>当前序列被标为废片，原因：' + str + '</span>');
                }
                if (value.data.list.length != 0) {
                    var temp = {};
                    var seriesInstanceUids = [];
                    ...
                    temp.seriesInstanceUids = seriesInstanceUids;
                    temp.studyInstanceUid = temp.seriesInstanceUids[0].seriesInstanceUid;
                    _this4.model.setData('series_result_id', temp);
                } else {
                    _this4.dom.find('.back-icon').click();
                }
            });
        }
        ...
        value: function startTask() {
            ...
            this.api.series_get({
                ...
                var serLength = value.data.sarIdList.length;
                var seriesInstanceUids = [];
                ...
                if (!_this5.model.taskInfo.studyAnno) {
                    endInfo.studyInstanceUid = endInfo.seriesInstanceUids[0].seriesInstanceUid;
                }
                _this5.model.setData('series_result_id', endInfo);
            });
        }
        ...
        value: function result_get_fromback() {
            ...
            var lists = this.model.series_result_id.seriesInstanceUids;

            var _loop = function _loop(i) {
                _this6.api.series_result_read({
                    id: lists[i].seriesAnnotationResultId
                }).done(function (res) {
                    _this6.model.seriesResult[lists[i].seriesAnnotationResultId] = res;
        ...
        value: function cornerstoneContorlOther() {
            ...
            this.cornerstoneContorl.event._addEvent('ctcornerstone.addNode', function (value) {
                ...
                var handles = value.handles ? JSON.parse(JSON.stringify(value.handles)) : {
                    start: {},
                    end: {}
                };
                ...
                //console.log(value)
                var seriesInfoData = that.model.seriesInfo[value.sId].info.data;
                var nee = false;
            ...
            this.cornerstoneContorl.event._addEvent('ctcornerstone.Allthing', function (value) {
                that.app.loading.show();
                var ids = that.getSarIds();
                var data = that.model.changeItemDataToBackend(that.model.taskInfo.id, value);
            ...
            this.cornerstoneContorl.event._addEvent('ctcornerstone.makeMagic', function (value) {
                ...
                var sId = value.sId;
                var dataToolType = that.dicommenu.chooseData.data;
                var infoData = that.model.seriesInfo[sId];
                ...
                that.app.alert.show({
                    ...
                    sure: function () {
                        var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
                            var ctVal, points, backData, magicloadingClass, i, j, resCreate, resRead;
                                ...
                                while (1) {
                                    switch (_context.prev = _context.next) {
                                        case 0:
                                            ...
                                            //return
                                            ctVal = isNaN(that.app.alert.dom.find('.ct_d').val()) ? 0 : that.app.alert.dom.find('.ct_d').val() * 1;

                                            that.model.loadData = {
                                                total: 1,
                                                num: 0
                                            };
                                            points = value.e.detail.currentPoints.image;
                                            ...
                                            magicloadingClass = __webpack_require__(645);

                                            that.magicloading = that.app.loadModal(magicloadingClass, {
                                                adv: true
                                            });
                                            _context.t0 = regeneratorRuntime.keys(that.model.seriesResult);
                                        ...
                                        case 9:
                                            ...
                                            i = _context.t1.value;
                                            _context.t2 = regeneratorRuntime.keys(that.model.seriesInfo);
                                        ...
                                        case 12:
                                            ...
                                            _context.prev = 15;
                                            _context.next = 18;
                                            return that.api.magicToolCerate({
                                                sarId: i,
            ...
            //编辑魔法棒
            this.cornerstoneContorl.event._addEvent('ctcornerstone.editBrush', function () {
                var _ref2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(value) {
                    var infoData, _base, base64, submitData, res;
                    ...
                    return regeneratorRuntime.wrap(function _callee2$(_context2) {
                        while (1) {
                            switch (_context2.prev = _context2.next) {
                                case 0:
                                    console.log(value);
                                    infoData = that.model.seriesInfo[value.sId];
                                    ...
                                    that.app.loading.show();
                                    _base = value.image.src.replace('data:image/png;base64,', '');
                                    ...
                                    if (infoData.info.data.fileType == "DCM" && (infoData.info.data.modality == 'DX' || infoData.info.data.modality == 'DR' || infoData.info.data.modality == 'CR')) {
                                        ...
                                        that.changeImage(value.image.src).then(function (value) {
                                            _base = value;
                                        });
                                    }
                                    setTimeout(function () {
                                ...
                                case 8:
                                    base64 = Tool.changeImageToBase64(value.image);
            ...
            this.cornerstoneContorl.event._addEvent('ctcornerstone.addImageResult', function (value) {
                ...
                //return
                that.app.loading.show();
                var base64 = value.image.src.replace('data:image/png;base64,', '');
                var infoData = that.model.seriesInfo[value.sId];
                if (infoData.info.data.fileType == "DCM" && (infoData.info.data.modality == 'DX' || infoData.info.data.modality == 'DR' || infoData.info.data.modality == 'CR')) {
                    that.changeImage(value.image.src).then(function (value) {
                        base64 = value;
                    });
        ...
        value: function needApiAll(data, apineed, newdata, postData) {
            ...
            if (apineed) {
                this.app.loading.show();
                this.api.image_result_create(postData).done(function (res) {
                    _this7.app.loading.hide();
                    newdata.id = data.backId = res.data.id;
                    data.brush = true;
                    console.log(data, 'data');
                    _this7.cornerstoneContorl.updataSequencelist(data);
                    newdata.id = res.data.id;
                    _this7.cornerstoneContorl.setbrush(newdata);
                });
            } else {
        ...
        value: function btnEvent() {
            ...
            this.dom.find('.back-icon').on('click', function () {
                window.location.href = 'http://' + window.location.host + '/#!/taskdetail/' + _this8.app.parpam['taskId'] + '/' + _this8.app.parpam['rid'];
                window.location.reload();
                return;
            });
            var that = this;
        ...
        key: 'submitSeries',
        value: function submitSeries(id, pading) {
            var _this9 = this;
            ...
            this.api.series_result_read({
                ...
                _this9.app.loading.hide();
                _this9.needSub[vid] = false;
                if (value.data.yayAttributes) {
                    if (_this9.model.projectInfo.yayAttributes * 1 == 4) {
                        _this9.startSubmit();
                        return false;
                    }
                    if (_this9.model.projectInfo.yayAttributes * 1 == 3 && value.data.yayAttributes * 1 == 1) {
                        _this9.startSubmit();
                        return false;
                    }
                    if (_this9.model.projectInfo.yayAttributes * 1 == 1 && value.data.yayAttributes * 1 == 2) {
                        _this9.startSubmit();
                        return false;
                    }
                }
                var data = value.data;
                var conmponentInfo = _this9.cornerstoneContorl.model.nidusComponentInfo;
                //判断大征象是否完成了
                var allnid = _this9.allnidussubmit(data);
                if (!allnid) {
                    return true;
                }
                console.log(conmponentInfo, 'conmponentInfoconmponentInfoconmponentInfo');
                console.log(_this9.needSub, vid);
                _this9.needSub[vid] = false;
                console.log(data);
                for (var i = 0; i < data.imageAnnotationResultList.length; i++) {
                    var item = data.imageAnnotationResultList[i];
                    var optionLength = 0;
                    for (var wn in conmponentInfo[item.imageAnnotationType]) {
                        if (conmponentInfo[item.imageAnnotationType][wn].optional) {
                            optionLength++;
                        }
                    }
                    if (item.annotationItemResultList == null && optionLength != 0) {
                        for (var w in conmponentInfo[item.imageAnnotationType]) {
                            if (conmponentInfo[item.imageAnnotationType][w].optional) {
                                _this9.errorShow(item, conmponentInfo[item.imageAnnotationType][w]);
                                _this9.needSub[vid] = true;
                                return true;
                            }
                        }
                    }
                    ...
                    if (item.annotationItemResultList) {
                        ...
                        if (optionLength) {
                            var idsAll = ',';
                            for (var j = 0; j < item.annotationItemResultList.length; j++) {
                                var res = item.annotationItemResultList[j];
                                if (res.result) {
                                    idsAll += res.annotationItemId + ',';
                                }
                            }
                            for (var _w in conmponentInfo[item.imageAnnotationType]) {
                                if (idsAll.lastIndexOf(',' + _w + ',') == -1 && conmponentInfo[item.imageAnnotationType][_w].optional) {
                                    _this9.errorShow(item, conmponentInfo[item.imageAnnotationType][_w]);
                                    _this9.needSub[vid] = true;
                                    return true;
                                }
                            }
                        ...
                        for (var _j = 0; _j < item.annotationItemResultList.length; _j++) {
                            var _res = item.annotationItemResultList[_j];
                            if (!_res.result && conmponentInfo[item.imageAnnotationType][_res.annotationItemId].optional) {
                                _this9.needSub[vid] = true;
                                _this9.errorShow(item, conmponentInfo[item.imageAnnotationType][_res.annotationItemId]);
                                return true;
                            }
                        }
                    }
        ...
        key: 'allnidussubmit',
        value: function allnidussubmit(data) {
            var _this10 = this;

            var conmponentInfo = this.cornerstoneContorl.model.nidusComponentInfo;
            var now = false;
            //console.log(conmponentInfo.all)

            var _loop2 = function _loop2(i) {
                if (i != 'annotationItemResultList') {
                    var allitem = conmponentInfo.all[i];
                    if (allitem.optional) {
                        var nowS = false;
                        data.annotationItemResultList.map(function (item) {
                            if (item.annotationItemId == i && item.result) {
                                nowS = true;
                            }
                        });
                        if (!nowS) {
                            _this10.alerError('“序列整体标注信息”的“' + allitem.componentName + '”为必填项，请填写完整。');
                            return {
                                v: false
                            };
                        }
            ...
            for (var i in conmponentInfo.all) {
                var _ret2 = _loop2(i);

                if ((typeof _ret2 === 'undefined' ? 'undefined' : _typeof(_ret2)) === "object") return _ret2.v;
            }
            if (data.annotationItemResultList.length == 0) {
                for (var ww in conmponentInfo.all.annotationItemResultList) {
                    console.log(conmponentInfo.all);
        ...
        value: function series_submit() {
            ...
            this.api.series_result_submit({
                ...
                _this11.app.loading.hide();
                console.log(_this11.model.doneNum);
                window.location.href = 'http://' + window.location.host + '/#!/taskdetail/' + _this11.app.parpam['taskId'] + '/' + _this11.app.parpam['rid'];
                window.location.reload();
                return;
                if (_this11.model.doneNum == -1) {
                    window.location.href = 'http://' + window.location.host + '/#!/personalaccount';
                    window.location.reload();
        ...
        value: function errorShow(item, node) {
            ...
            var msg = temp[item.imageAnnotationType] + '_' + item.id;
...
module.exports = "<div class=mark> <ul class=mark-header> <li class=\"back-icon fl\"> <span><span class=lefta><i class=\"iconfont icon-icon-test3\" style=font-size:12px></i></span><span class=infojian>返回</span></span> </li> <li class=\"task-name fl\"> <span>|</span> <span>Cassiel0705标注4</span> </li> <li class=\"cotrol-btn fr\"> <div class=\"btn btn-submit\">提交</div> </li> <li class=\"discard_info fr\" style=margin-right:10px></li> </ul> <div class=mark-menu></div> <div class=mark-content> </div> </div>";
...
//# sourceMappingURL=dfa30f06e283f5c1d44d.20.min.js.map