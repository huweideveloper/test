...
var markmodel = function (_MarkModel) {
    ...
    _createClass(markmodel, [{
        ...
        value: function changeItemDataToBackendCheck(value) {
            var postData = [];
            console.log(value, 'changeItemDataToBackendchangeItemDataToBackend');
...
var markaudit = function (_MarkBase) {
    _inherits(markaudit, _MarkBase);

    function markaudit(app, api, dom, model) {
        _classCallCheck(this, markaudit);

        return _possibleConstructorReturn(this, (markaudit.__proto__ || Object.getPrototypeOf(markaudit)).call(this, app, api, dom, model));
    }
    //接口调用，获取信息


    _createClass(markaudit, [{
        key: 'baseApi',
        value: function baseApi() {
            ...
            this.api.task_read({
                ...
                _this2.imageDataType = String(value.data.seriesImgFileType) == "2" ? "orgin" : "noneOrgin";
        ...
        value: function annotationIteminit() {
            ...
            this.app.parpam['taskInfo'] = Base64.decode(this.app.parpam['taskInfo']);
            ...
            if (this.app.parpam['type'] == 'check_viewer' || this.app.parpam['type'] == 'check_viewer_taskdetail' || this.app.parpam['type'] == 'check_viewer_taskdetail_view') {
                var taskData = this.app.parpam['taskInfo'].split('$$');
                var temp = {
        ...
        value: function series_get(sid) {
            ...
            this.api.series_get({
                ...
                if (value.code == 1001) {
                    window.location.href = 'http://' + window.location.host + '/#!/viewaudittask/' + _this4.app.parpam['taskId']; //this.app.changePage('personalaccount')
                    window.location.reload();
                }
                if (!value.data) {
                    window.location.href = 'http://' + window.location.host + '/#!/viewaudittask/' + _this4.app.parpam['taskId'];
                    window.location.reload();
                }
                var temp = {
        ...
        value: function cornerstoneContorlOther() {
            ...
            this.cornerstoneContorl.event._addEvent('ctcornerstone.editNodeItem', function (value) {
                ...
                //return
                //console.log(value.chooseData)
                var arr = that.model.changeItemDataToBackend(value.chooseData);
                that.app.loading.show();
        ...
        value: function btnEvent() {
            ...
            this.dom.find('.back-icon').on('click', function () {
                if (_this5.app.parpam['type'] == 'check_viewer_taskdetail') {
                    _this5.app.changePage('taskdetail', { taskId: _this5.app.parpam['taskId'], type: 'editor' });
                    // window.location.href = 'http://' + window.location.host + '/#!/taskdetail/' + this.app.parpam['taskId'] + '/editor'
                    // window.location.reload()
                    return;
                }
                if (_this5.app.parpam['type'] == 'check_viewer_taskdetail_view') {
                    _this5.app.changePage('taskdetail', { taskId: _this5.app.parpam['taskId'], type: 'view' });
                    // window.location.href = 'http://' + window.location.host + '/#!/taskdetail/' + this.app.parpam['taskId'] + '/view'
                    // window.location.reload()
                    return;
                }
                _this5.app.changePage('viewaudittask', { id: _this5.app.parpam['taskId'] });
                // window.location.href = 'http://' + window.location.host + '/#!/viewaudittask/' + this.app.parpam['taskId']
                //   window.location.reload()
            });
            var that = this;
            if (this.app.parpam['type'].lastIndexOf('viewer_all') == -1) {
                ...
                var ti = this.app.parpam['taskInfo'].split('$$');
                this.dom.find('.cotrol-btn .btn-submit1').on('click', function () {
                    //console.log('aaaa')
                    if (ti[1] * 1 > 0) {
                        var nowtaksInfod = ti[0] + '$$' + (ti[1] - 1);
                        _this5.app.changePage('markauditview', { taskId: _this5.app.parpam['taskId'], taskInfo: nowtaksInfod, type: 'check_viewer_all' });
                        // window.location.href = 'http://' + window.location.host + '/#!/markauditview/' + this.app.parpam['taskId'] + '/' + nowtaksInfod + '/check_viewer_all'
                        //   window.location.reload()
                    } else {
                        _this5.alerError('没有上一个序列了！');
                    }
                });
                this.dom.find('.cotrol-btn .btn-submit2').on('click', function () {
                    if (ti[1] * 1 < ti[0] - 1) {
                        var nowtaksInfod = ti[0] + '$$' + (ti[1] * 1 + 1);
                        _this5.app.changePage('markauditview', { taskId: _this5.app.parpam['taskId'], taskInfo: nowtaksInfod, type: 'check_viewer_all' });
                        // window.location.href = 'http://' + window.location.host + '/#!/markauditview/' + this.app.parpam['taskId'] + '/' + nowtaksInfod + '/check_viewer_all'
                        //   window.location.reload()
                    } else {
                        _this5.dom.find('.back-icon').click();
                    }
                });
            }
        }
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./markauditview.less", function() {
			var newContent = require("!!../../node_modules/css-loader/index.js!../../node_modules/less-loader/dist/cjs.js!./markauditview.less");
...
var api = {
    ...
    series_result_next: function series_result_next(value) {
        var deferred = ES.Deferred();
        ES.ajax({
    ...
    task_study_result_read: function task_study_result_read(value) {
        var deferred = ES.Deferred();
        ES.ajax({
...
module.exports = "<div class=markaudit> <ul class=mark-header> <li class=\"back-icon fl\"> <span><span class=lefta><i class=\"iconfont icon-icon-test3\" style=font-size:12px></i></span>返回</span> </li> <li class=\"task-name fl\"> <span>|</span> <span></span> </li> <li class=\"discard_info fr\" style=margin-right:10px></li> <li class=\"cotrol-btn fr\"> <div class=\"btn btn-submit1\">上一个序列</div> <div class=\"btn btn-submit2\">下一个序列</div> </li> </ul> <div class=mark-menu></div> <div class=mark-content> </div> </div>";
...
//# sourceMappingURL=1567acbb62e2bea309ab.10.min.js.map