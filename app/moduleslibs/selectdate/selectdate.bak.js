    ...
    this.complete = function() {
        ...
        this.dom.find('.ranges .btn-success').on('click', function() {
            ...
            if (that.nowParam.opener) {
                that.nowParam.opener.val(dealZero(that.chooseStart) + '至' + dealZero(that.chooseEnd))
            }
            that.event._dispatch('day.picker', { st: dealZero(that.chooseStart), et: dealZero(that.chooseEnd), tempDate: that.tempDate,pid:that.nowParam.id })
        })
    ...
    this.initData = function(data, el) {
        ...
        this.dom.find('.daterangepicker .' + el + ' td').on('click', function() {
            ...
            if (that.mode == 's') {
                that.dom.hide()
                if (that.nowParam.opener) {
                    that.nowParam.opener.val(dealZero(that.chooseStart))
                }
                that.event._dispatch('day.picker', { st: dealZero(that.chooseStart), tempDate: that.tempDate,pid:that.nowParam.id})
                that.event._dispatch('day-mask.click')
            } else {
