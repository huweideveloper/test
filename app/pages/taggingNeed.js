class taggingNeed extends DataBase {
  complete() {
    let that = this
    let permission = this.app.userResource[
      ES.selctorDoc('.menu .twolink[link="taggingNeed"]')
        .parent()
        .attr("id")
    ]
    ...
    this.model.tableconfig.tablewidth =
      ES.selctorDoc(".taggingNeed").box().clientWidth - 140
    this.loadlist("group")
    this.addUser1 = require("../modal/addUser1/addUser")
    this.apidata = {}
    //新增用户
    this.dom.find(".createbtn").on("click", async () => {
      this.app.changePage("taggingNeedDetail", { type: "new" })
    })
  ...
  async listaction(value) {
    switch (value.classname) {
      case "EDIT":
        this.app.changePage("taggingNeedDetail", { type: "edit", id: value.id })
        break
      case "READ":
        this.app.changePage("taggingNeedDetail", { type: "view", id: value.id })
        break
      case "START":
        this.app.alert.show({
          title: " ",
