...
class filterNeedDetail extends Interstellar.pagesBase {
  complete() {
    this.styleModel(1);
    this.getdictionary();
  ...
  render() {
    ...
    this.dom.find(".back").on("click", () => {
      this.app.changePage("filterNeed");
    });
    ...
    this.dom.find(".batchExport").on("click", async () => {
      ...
      if (tempArr.length > 0) {
        let url =
          this.app.domain1 +
          "v1/tagneed/convertList/exportV2?tagNeedId=" +
          this.id +
          "&tagTransferIdList=" +
          tempArr.join(",") +
          "&accessToken=" +
  ...
  setTitle() {
    ...
    this.model.listheader[this.type].tablewidth =
      ES.selctorDoc(".filterNeedDetail").box().clientWidth - 40;
        ...
        switch (value.classname) {
          case "start":
            this.app.alert.show({
              ...
              sure: async () => {
                this.app.loading.show();
                let res1 = await this.api.tagneed_batchExecuteTagTransfer({
                  idsList: [value.id * 1]
                });
                this.app.loading.hide();
                if (res1.code == 0) {
      ...
      default:
        this.dom.find(".save").on("click", async () => {
          ...
          if (
            ...
            let res = await this.api.tagneed_addTagNeed({ ...this.model.apiData, type: 2});
            if (res.code == 0) {
              this.app.changePage("filterNeed");
            } else {
              Tool.errorshow(res.msg, this.app);
  ...
  loadList(list, bool, pages, total) {
    if (list.length > 0) {
      ...
      list.forEach((val, idx) => {
        ...
        const listicon = this.model.listicon[this.type]
        if (this.type == "new") {
          val.operation = listicon.action;
        } else {
          switch (val.status) {
            case 0:
              val.status = "未触发";
            ...
            case 1:
              val.status = "已触发";
              if (val.canRestart) {
                val.operation = listicon.action2;
              } else {
                val.operation = listicon.action1;
              }
              break;
            // case 2:
            //   val.status = "已完成";
            //   val.operation = { ...listicon.action4 }
            //   if (val.canRestart) {
            //     val.operation = { ...listicon.action6, ...val.operation };
            //   }
            //   break;
            // case 4:
            //   val.status = "已暂停";
  ...
  initscroll() {
    ...
    this.dom.find(".filterNeedDetail .revertTable").attr("id", rid);
  ...
  resize() {
    ...
    ES.selctorDoc(".filterNeedDetail").css({
      height: ch - 90,
      width: cw
    });
    this.dom
      .find(".filterNeedDetail .revertTable")
