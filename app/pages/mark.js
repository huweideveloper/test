...
class markview extends MarkBase {
    ...
    apiload() {
        ...
        //this.api.image_result_delete({id:10227})
    }
    ...
    startTask() {
        this.api.series_get({
            ...
            const { seriesAnnotationResultId, seriesInstanceUid } = endInfo.seriesInstanceUids[0]
    ...
    cornerstoneContorlOther() {
        let that = this
            //对于图片上面的影像标注操作
        this.cornerstoneContorl.event._addEvent('ctcornerstone.addImageTag', (value) => {
            console.log(value, that.model.seriesInfo[value.sId].sarId, value.sId, value.toolType[0].imageAnnotationId, value.toolType[0].id)
            if (that.model.seriesInfo[value.sId].sarId) {
                let postData = {}
                ...
                postData.imageAnnotationId = value.toolType[0].imageAnnotationId
                postData.imageAnnotationToolId = value.toolType[0].id
                postData.result=JSON.stringify({"z":value.layers})
                console.log(postData)
                return
                that.api.image_result_create(postData).done(function(res) {
                    that.app.loading.hide()
                    value.backId = res.data.id
                    that.cornerstoneContorl.updataImageTag(value)
                    console.log(value)
            ...
            if (value.toolType.type == 'POLYGON' || value.toolType.type == 'QSELECT' || value.toolType.type == 'REGION_PAINT' || value.toolType.type == 'FREEHAND') {
                ...
                setTimeout(() => {
                    that.api.anno_iar_update({
                        ...
                        that.app.loading.hide()
                        that.cornerstoneContorl.updataMaskInfo()
                    })
                }, 500)
    ...
    allnidussubmit(data) {
        ...
        console.log(data.annotationItemResultList)
        if (data.annotationItemResultList.length == 0&&conmponentInfo.all) {
