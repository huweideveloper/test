class createprofour extends Interstellar.pagesBase {
  ...
  async project_basic_read() {
    ...
    if (res.code == 0) {
      this.projectBasicData = res.data
      // let toolArr = []
  ...
  async setMain(bool) {
    ...
    if (res.code == 0) {
      if (res.data.length > 0) {
        // seriesImgFileType 9 随访配准
        this.projectBasicData.seriesImgFileType !== 9 && this.dom.find(".copytask").removeClass("hide")
        // 判断有没有批量导出权限start
